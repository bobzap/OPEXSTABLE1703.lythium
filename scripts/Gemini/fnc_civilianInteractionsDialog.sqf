...
// Amélioration de la robustesse des paramètres
if (isNil "_this") then {
    diag_log "ERROR: _this est nil dans fnc_civilianInteractionsDialog.sqf";
    _target = objNull;
    _caller = player;
} else {
    _target = _this param [0, objNull, [objNull]];
    _caller = _this param [1, objNull, [objNull]];
};

// Log de débogage
diag_log format ["civilianInteractionsDialog - Target: %1, Caller: %2", _target, _caller];

// Vérification de sécurité
if (isNull _target) exitWith {
    diag_log "ERROR: _target est objNull dans fnc_civilianInteractionsDialog.sqf";
    hint "Erreur: Cible invalide pour l'interaction";
    closeDialog 0;
};
...
private
    [
        ...
        "_civilian_chief_TITLE", "_civilian_chief_missions", "_civilian_chief_status", "_civilian_chief_reputation"
    ];
...
_civilian_chief_TITLE = 3030;
_civilian_chief_missions = 3031;
_civilian_chief_status = 3032;
_civilian_chief_reputation = 3033;
...
// CHEF DE VILLAGE - Définir les textes sans vérification supplémentaire
ctrlSetText [_civilian_chief_TITLE, toUpper localize "STR_dialog_civilianInteractions_chief_TITLE"];
ctrlSetText [_civilian_chief_missions, localize "STR_dialog_civilianInteractions_chief_missions"];
ctrlSetText [_civilian_chief_status, localize "STR_dialog_civilianInteractions_chief_status"];
ctrlSetText [_civilian_chief_reputation, localize "STR_dialog_civilianInteractions_chief_reputation"];
...
while {dialog} do {
    ...
    if (animationState _target != "AmovPercMstpSsurWnonDnon") then {
        ...
        if ("Gemini_bioscan" in ((uniformItems _caller) + (backpackItems _caller) + (vestItems _caller))) then {
            ctrlEnable [_civilian_IDcheck_scan, true]
        } else {
            ctrlEnable [_civilian_IDcheck_scan, false]
        };
        ctrlEnable [_civilian_suspect_down, true];
        ctrlEnable [_civilian_suspect_search, true];
        if (("Gemini_cableTie" in ((uniformItems _caller) + (backpackItems _caller) + (vestItems _caller))) || 
            ("ACE_CableTie" in ((uniformItems _caller) + (backpackItems _caller) + (vestItems _caller)))) then {
            ctrlEnable [_civilian_suspect_handcuff, true]
        } else {
            ctrlEnable [_civilian_suspect_handcuff, false]
        };
        if ((!(isClass (configFile >> "CfgPatches" >> "Gemini_items"))) && 
            (!(isClass (configFile >> "CfgPatches" >> "ace_main")))) then {
            ctrlEnable [_civilian_suspect_handcuff, true]
        };
    ...
    if (_target in attachedObjects _caller) then {
        ...
        ctrlEnable [_civilian_suspect_handcuff, false];
        ctrlEnable [_civilian_suspect_take, false];
        if (count (((position _target) nearEntities [["Air", "Car", "Motorcycle", "Tank"], 7.5])) > 0) then {
            ctrlEnable [_civilian_suspect_load, true]
        };
    ...
    if (_target distance _caller > 10) then {
