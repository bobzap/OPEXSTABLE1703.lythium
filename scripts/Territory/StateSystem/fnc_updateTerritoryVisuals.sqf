/*
    ...
    Usage: [_territoryIndex] call Gemini_fnc_updateTerritoryVisuals
    Retourne: bool
*/
...
if (count _markers > 1) then {
    ...
    private _markerText = switch (_state) do {
        case "unknown": { format ["%1 (Non renseigne)", _name] };
...
if (OPEX_territory_debug) then {
    diag_log format ["[TERRITOIRE][VISUALS] Marqueurs mis a jour pour %1: etat=%2, couleur=%3", _name, _state, _color];
};
